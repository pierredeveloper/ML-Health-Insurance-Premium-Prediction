...
def format_inr(amount):
    ...
    return f"₹ {amount:,.0f}".replace(",", "_").replace("_", ",", 1)
...
# Parse the medical history
    diseases = medical_history.lower().split(" & ")
# Maps them to predefined risk values and Sums those risks
    total_risk_score = sum(risk_scores.get(disease, 0) for disease in diseases)

    max_score = 14  # 8 (heart disease) + 6 (diabetes or BP)
    min_score = 0
...
def handle_scaling(age, df):
    ...
    # Dummy column required by scaler
...
def predict(input_dict):
    ...
    # Return formatted Indian currency
    return format_inr(predicted_cost)
